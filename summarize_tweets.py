...
def build_overall_prompt(chunk_summaries: Sequence[str]) -> List[dict]:
    ...
    prompt = textwrap.dedent(
        f"""
        你是一个专业的推特内容分析师，你重点关注 AI 领域的最新消息。
        请综合以下分段摘要，完成最近一小时的趋势分析，必须严格按照下述模板输出：

        1. top趋势关键词：列出 3~5 个具体的热门关键词/话题，避免笼统词汇（如 “AI”“Web3”），并在括号中简述出现原因或相关产品。
        2. 提到的 AI 产品及亮点：按 “产品名称，提及次数，亮点简述” 的形式列出 3~5 个，次数可根据摘要中的描述估算。
        3. 使用技巧 / 教程 / 实战案例：用项目符号归纳 3~5 条，说明是谁分享了什么做法、带来什么价值。
        4. 最有价值推文 TOP10：
           1. 推文概述（包含观点/用法/价值，必要时指出作者或账号特征）
           2. …
           3. …
           …
           10. …
